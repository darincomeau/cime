    ...
    def get_compset_components(self):
        ...
        for element in elements:
            ...
            if element.startswith("BGC%") or element.startswith("TEST"):
                ...
                if "ww" not in element_component and "fv3" not in element_component and "cice" not in element_component:
...
directory, NOT in this subdirectory."""
    ...
    def get_mpirun_cmd(self, job=None, allow_unresolved_envvars=True, overrides=None):
        ...
        if executable is not None and "aprun" in executable and not "theta" in self.get_value("MACH") and not "onyx" in self.get_value("MACH") and not "warhawk" in self.get_value("MACH") and not "blackbird" in self.get_value("MACH"):
